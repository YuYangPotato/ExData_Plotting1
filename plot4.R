...
voltage1 <- as.numeric(as.character(mydata2$Voltage))
zv <- data.frame(x, voltage1)
...
##data for graph4 
reactive <- as.numeric((as.character(mydata2$Global_reactive_power)))
re_vol <- data.frame(x, reactive)



##set up for the x-axis
v1 <- c(0, 1440, 2880)
v2 <- c("Thu", "Fri", "Sat")
...
plot(zv,type = "l", xlab = "datatime", ylab = "Voltage", xaxt = "n")
axis(side = 1, at = v1, labels = v2, tck = -.05)
...
plot(re_vol, type = "l", xlab = "datatime", ylab = "Global_reactive_power", xaxt = "n")
axis(side = 1, at = v1, labels = v2, tck = -.05)
